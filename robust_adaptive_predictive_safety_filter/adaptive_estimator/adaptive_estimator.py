...
class AdaptiveEstimator:
    def __init__(self, theta_val, f, window=20, Theta=None, nx=1, nu=1, integrator_type='cont', dT = 0.1, params={}, sampling_time=1, initial_theta_bound=None):
        ...
        self.initial_theta_bound = initial_theta_bound
    ...
    def setup(self):
        ...
        self.Ldk =  self.f.model_error(0, self.theta_error)
        self.output_Ld = self.Ldk
    ...
    def computeCLLyapunov(self):
        ...
        if self.initial_theta_bound:
            opti.set_initial(theta1, self.initial_theta_bound['theta1'])
            opti.set_initial(theta2, self.initial_theta_bound['theta2'])
    ...
    def __call__(self, x, k, u):
        ...
        if len(self.x) < self.window + 1:
            ...
            theta, self.Ldk = self.concurrent_learning()
            print('(adaptive_estimator) updating parameters....')
            ...
            if k % self.sampling_time == 0:
                ...
                self.output_Ld = self.Ldk
                print('adaptive_estimator) updating output parameters')
    ...
    def replace_cl_data(self):
        ...
        xp1 = np.array(self.x[-1]).reshape((self.nx,1))
        ...
        Znew_np = np.array(Znew[-self.window:]).reshape((self.f.ntheta, -1))

        print('self.Z = '+str(self.Z))
        print('Znew = ' +str(Znew[-self.window:]))
        ...
        print('new_conv_ratio = '+str(new_conv_ratio))
        print('rankZnew = '+str(rankZnew))
        print('l3 = '+str(l3))
        print('l4 = '+str(l4))
        ...
        print('conv ratio = '+str(self.conv_ratio))
