...
if __name__ == "__main__":
    ...
    model_list = [(amin + (amax - amin)*torch.rand(1,1))*torch.ones(params['nsteps']+1, nmodel) for k in range(params['n_samples'])]
    ...
    model = Node(f, ['x', 'u', 'a'], ['x'], name='model')
    ...
    a = torch.tensor( (amin + (amax - amin)  * torch.rand(1,1))*torch.ones(nsteps+ 1, nmodel), dtype=torch.float32).reshape(1, nsteps+1, nmodel)
    print('a = '+str(a[0,0,0]))
    ...
    #torch.save(best_model, 'single_integrator_adaptive_policy_'+str(params['policy_number'])+'.pth')
    #torch.save(params, 'single_integrator_adaptive_params_'+str(params['policy_number'])+'.pth')