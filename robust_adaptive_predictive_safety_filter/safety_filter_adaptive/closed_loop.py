    ...
    def simulate(self, x0, N):
        ...
        for kk in tqdm(range(N)):
            ...
            try:
                ...
                print(traceback.format_exc())
                print('WARNING: Control computation failed, exiting closed-loop (adaptive) simulation')
                print('x = '+str(x))
                print('kk = '+str(kk))
                print('unom = ' + str(self.u_nom(x, kk)))
                return X,U

            # Compute estimator of model parameters and update nominal control
            theta, Ld = self.adaptive_estimator(x, kk, uk)
            print('theta = '+str(theta))
            print('Ld from adaptive estimator = '+str(Ld))
            print('Ldk from ae = '+str(self.adaptive_estimator.Ldk))
            print('theta error from ae = '+str(self.adaptive_estimator.theta_error))
            conv_cond = self.adaptive_estimator.check_convergence_condition()
            print('conv cond = '+str(conv_cond))
            len_theta = len(theta)
            ...
            self.u.update_model(theta, Ld)

            try:
                print('N = '+str(self.u.N))
            except:
                pass
...
class DummyNominalControl:
    def __init__(self, unom):
        self.unom = unom

    def __call__(self, x, k):
        return self.unom(x,k)

    def update_exogenous_variables(self, input):
        pass
