...
from safetyfilter import NNModel
...
if __name__ == "__main__":
    ...
    for kk in range(Ntotal_sim):
        ...
        x01 = random.uniform(0.2, 1.0)
        x02 = random.uniform(0.2, 1.0)
        ...
        #u_nom = lambda x,k: np.array([(0.1+a1)*sin(x[0]) + (0.5 + b1), (0.1 + a2)*sin(x[1]) + (0.5 + b2)])
        ...
        AE = AdaptiveEstimator(theta_val=theta0, f=model_adaptive, type='ConcurrentLearning', window=100,
                               Theta=theta_bounds, nx=nx, nu=nu, dT=dt, cl_params=cl_params)
