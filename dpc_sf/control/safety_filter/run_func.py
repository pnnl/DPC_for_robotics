...
def run_p2p_no_sf(Ts, Ti, Tf, save_dir):
    ...
    if Ts == 0.01:
        np.savez(
            save_dir + "dpc_obs_avoid_10ms.npz", 
            x_history=ptu.to_numpy(data['X'])[0,...], 
            u_history=ptu.to_numpy(data['U'])[0,...]
        )
    elif Ts == 0.001:
        np.savez(
            save_dir + "dpc_obs_avoid_1ms.npz", 
            x_history=ptu.to_numpy(data['X'])[0,...], 
            u_history=ptu.to_numpy(data['U'])[0,...]
        )

def run_p2p(Ts, Ti, Tf, save_dir):
    ...
    if Ts == 0.01:
        np.savez(
            save_dir + "dpc_sf_obs_avoid_10ms.npz", 
            x_history=ptu.to_numpy(data['X'])[0,...], 
            u_history=ptu.to_numpy(data['U'])[0,...]
        )
    elif Ts == 0.001:
        np.savez(
            save_dir + "dpc_sf_obs_avoid_1ms.npz", 
            x_history=ptu.to_numpy(data['X'])[0,...], 
            u_history=ptu.to_numpy(data['U'])[0,...]
        )

    # np.savez("dpc_obs_avoid.npz", x_history=ptu.to_numpy(data['X'])[0,...], u_history=ptu.to_numpy(data['U'])[0,...])
