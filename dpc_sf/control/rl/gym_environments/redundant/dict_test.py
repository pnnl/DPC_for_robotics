...
class QuadcopterGymP2P(Env):
    ...
    def __init__(
        ...
        state_ub = params['state_ub'].astype(sa_dtype)
        state_lb = params['state_lb'].astype(sa_dtype)
        cyl_ub = np.array([1.5, 1.5, 0.5], dtype=sa_dtype)
        cyl_lb = np.array([0.5, 0.5, 0.2], dtype=sa_dtype)

        # must instead stack the reference and the observation
        self.observation_space = spaces.Dict(spaces={
            'agent': spaces.Box(low=state_lb, high=state_ub, dtype=sa_dtype),
            'desired_goal': spaces.Box(low=state_lb, high=state_ub, dtype=sa_dtype),
            'obstacle': spaces.Box(low=cyl_lb, high=cyl_ub, dtype=sa_dtype),
            'achieved_goal': spaces.Box(low=state_lb, high=state_ub, dtype=sa_dtype)
        })        
    ...
    def _get_obs(self):
        ...
        return {'agent': state, 'desired_goal': ref, 'obstacle': self.cylinder_params, 'achieved_goal': state}
    ...
    def _cost(self, achieved_goal, desired_goal, _info):
        state=achieved_goal
        delta_state = desired_goal - achieved_goal

        # quadratic cost - ignore inputs
        cost = delta_state @ self.Q @ delta_state
    ...
    def compute_reward(self, state, ref, _info):
        return - self._cost(achieved_goal=state, desired_goal=ref, _info=_info)
    ...
    def step(self, cmd: np.ndarray):
        ...
        reward = self.compute_reward(state=self.quad.state, ref=self.quad.reference(self.quad.t), _info={})
