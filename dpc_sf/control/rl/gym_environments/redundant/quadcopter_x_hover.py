    ...
    * Environment Name: QuadrotorPlusHoverEnv-v0
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low = np.hstack([f32_state_lb[0:3], -np.ones(9), f32_state_lb[7:]]), 
            high = np.hstack([f32_state_ub[0:3], np.ones(9), f32_state_ub[7:]]), 
            dtype=sa_dtype
        )
    ...
    def _get_obs(self):
        ...
        return np.concatenate([e_pos, rot_mat.flatten(), vel, angular_vel, omegas]).flatten()
    ...
    def get_reward(self, ob, a):
        ...
        reward_orientation = self.orientation_error(self.quad.get_state()[3:7]) * (-self.orientation_reward_constant)
...
#         """
#         self._time = 0
#         random_state = self._random_state(randomize=self.randomize_reset)
