...
class TrajSafeSet:
    def __init__(self, data) -> None:
        ...
        print('preprocessing data...')
        safe_points = self.preprocess_data(data, num_points_considered)

        print('generating scipy convex hull...')
        self.hull = SPConvexHull(safe_points)
    ...
    def preprocess_data(self, data, num_points_considered):
        ...
        x = torch.stack(data['x'])
        u = torch.stack(data['u'])
        r = torch.stack(data['r'])
        e = r - x
        loss = torch.stack(data['loss'])

        log = {'success': [], 'failure': [], 'success_inputs': [], 'failure_inputs': [], 'minibatch_loss': [], 'minibatch_number': []}

        for i, (minibatch, minibatch_inputs) in enumerate(zip(e, u)):
            for trajectory, inputs in zip(minibatch, minibatch_inputs):
                # state = {x, xdot, y, ydot, z, zdot}
                err = trajectory[:,0:6:2][:-1]

                # what we care about is if the control input is towards the reference as that is the accereleration
                L = torch.einsum('ij,ij->i', err, inputs)
        ...
        # Select the points using the random indices
        safe_points = all_safe_points[random_indices, :]
...
if __name__ == "__main__":
    ...
    data = torch.load('large_data/traj_training_data.pt')
    ss = TrajSafeSet(data)

    data = torch.load('large_data/nav_training_data.pt')
    ss = NavSafeSet(data, generate_delaunay=True)
