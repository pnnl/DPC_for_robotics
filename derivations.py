...
from sympy.physics.mechanics import ReferenceFrame, Point, dynamicsymbols, inertia, RigidBody, dot, cross, KanesMethod, mprint

def derive_quadcopter_dynamics():
    ...
    wM1, wM2, wM3, wM4 = dynamicsymbols('wM1 wM2 wM3 wM4')
    ...
    wM1d, wM2d, wM3d, wM4d = dynamicsymbols('wM1 wM2 wM3 wM4', 1)

    # Constants
    # ---------------------------
    mB, g, dxm, dym, dzm, IBxx, IByy, IBzz, IRzz, u1, u2, u3, u4 = symbols('mB g dxm dym dzm IBxx IByy IBzz IRzz u1 u2 u3 u4')
    # ThrM1, ThrM2, ThrM3, ThrM4, TorM1, TorM2, TorM3, TorM4 = symbols('ThrM1 ThrM2 ThrM3 ThrM4 TorM1 TorM2 TorM3 TorM4')

    qW1, qW2, velW, Cd, KTh, KTo = symbols('qW1 qW2 velW Cd KTh KTo')

    ThrM1, ThrM2, ThrM3, ThrM4 = KTh * wM1**2, KTh * wM2**2, KTh * wM3**2, KTh * wM4**2
    TorM1, TorM2, TorM3, TorM4 = KTo * wM1**2, KTo * wM2**2, KTo * wM3**2, KTo * wM4**2
    ...
    # ---------------------------
    kd = [xdot - xd, ydot - yd, zdot - zd, q0d - quat_dot[0], q1d - quat_dot[1], q2d - quat_dot[2], q3d - quat_dot[3], wM1d - wM1dot, wM2d - wM2dot, wM3d - wM3dot, wM4d - wM4dot]

    # Kane's Method
    # --------------------------- considers wM1 wM2 wM3 wM4 as positions, and positions them in the state vector accordingly
    q_ind = [x, y, z, q0, q1, q2, q3, wM1, wM2, wM3, wM4]
    u_ind = [xdot, ydot, zdot, p, q, r]
    states = {str(x)[:-3]: x for x in q_ind + u_ind}
    inputs = {'u1': u1, 'u2': u2, 'u3': u3, 'u4': u4}
    ...
    return stateDot, states, inputs

def calculate_relative_degree(state_dot, states, inputs):

    state_variables = list(states.values())
    ...
    for i, key in enumerate(states):
        print(key)
        print(i)
        expression = state_dot[i].diff('t')
        for j, state in enumerate(state_variables):
            expression = expression.subs(state.diff('t'), state_dot[j])
        print(expression)
...
if __name__ == "__main__":
    ...
    state_dot, states, inputs = derive_quadcopter_dynamics()

    relative_degrees = calculate_relative_degree(state_dot, states, inputs)
