...
from dependency_free.high_ram_safe_set import SafeSet, directed_search
...
class SafetyFilter:
    ...
    def __call__(self, x, u, equation_cvx, equation_noncvx):
        ...
        u = ptu.to_numpy(u.T)
        x = ptu.to_numpy(x.T)
        equation_cvx = ptu.to_numpy(equation_cvx).flatten()
        equation_noncvx = ptu.to_numpy(equation_noncvx).flatten()
        ...
        if in_cvx_hull and in_noncvx_hull:
            ...
            # the convex hull
            self.opti_feas.set_value(self.a1_f, equation_cvx[:-1])
            self.opti_feas.set_value(self.b1_f, equation_cvx[-1])
            self.opti.set_value(self.a1, equation_cvx[:-1])
            self.opti.set_value(self.b1, equation_cvx[-1])

            # the pv convex hull
            self.opti_feas.set_value(self.a2_f, equation_noncvx[:-1])
            self.opti_feas.set_value(self.b2_f, equation_noncvx[-1])
            self.opti.set_value(self.a2, equation_noncvx[:-1])
            self.opti.set_value(self.b2, equation_noncvx[-1])
...
def run_nav_mj_many(
    ...
    ss = SafeSet(log, generate_delaunay=True)
    ...
    def nearest_halfspace(obs, ss=ss):
        ...
        # equation_cvx = equations[simplex]

        # equation_cvx = ss.is_in_cvx_hull_control(obs_np)
        # in_cvx_hull = equation_cvx_ctrl[:-1] @ obs_np.flatten() + equation_cvx_ctrl[-1] <= 0
        # if in_cvx_hull is True:
        #     equation_cvx = equation_cvx_ctrl
        # equation_cvx = equation_cvx_ctrl
        equation_cvx = np.array([0.,0,0,0,0,0,-100])

        # print(f"in_cvx_hull_ctrl: {in_cvx_hull}")
        equation_noncvx = ss.is_in_non_cvx_hull(obs_np)
        # pv = np.hstack(posVel2cyl.numpy_vectorized(obs_np, ss.cylinder_position, ss.cylinder_radius)).flatten()
        # if equation_noncvx[:-1] @ pv + equation_noncvx[-1] <= 0:
        #     equation_noncvx = np.array([0., 1., -100]) 
        return ptu.from_numpy(equation_cvx[None,:]), ptu.from_numpy(equation_noncvx[None,:])
    ...
    def filter(x_seq, u_seq, equation_cvx, equation_noncvx):
        u, x_planned = safety_filter(x_seq, u_seq, equation_cvx, equation_noncvx)
        return u.unsqueeze(0), x_planned
    filter_node = nm.system.Node(filter, input_keys=['X', 'U', 'Halfspace_cvx', 'Halfpsace_noncvx'], output_keys=['U_filtered', 'X_planned'], name='dynamics')
